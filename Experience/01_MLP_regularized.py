...
def train_model(model, 
    ...
    for epoch in range(min_epochs, epochs):
        ...
        for features, targets, batch in train_loader:
            ...
            loss1 = loss_fn(preds.view(targets.size()), targets)
            
            loss2 = lc_layer(batch, preds.view(-1,1) * (pi/180))
            loss2 = torch.mean((loss2 - 0)**2)
            
            loss = loss1 + loss2
            
            epoch_loss += loss.item()
            epoch_loss_data += loss1.item()
            epoch_loss_physics += loss2.item()
...
def eval_model(model, data_loader, loss_fn, lc_layer):
    model.eval()
    eval_loss = 0
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--load_path", help="the path from which the model should be loaded", required=False, default="trained_model/MLP_REG_14", type=str)
    ...
    parser.add_argument("-s", "--save_path", help="Path where the model should be saved", required=False, default="trained_model/MLP_reg_14", type=str)
